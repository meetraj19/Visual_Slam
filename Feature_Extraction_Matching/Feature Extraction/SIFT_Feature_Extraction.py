...
def extract_sift_features(images: List[Tuple[str, np.ndarray]], num_features: int = 1000) -> List[
    ...
    for image_name, image in images:
        ...
        if keypoints and descriptors is not None:
            ...
            print(f"No SIFT keypoints detected in {image_name}")
...
def save_features(features: List[Tuple[str, List[cv2.KeyPoint], np.ndarray]], output_dir: Union[str, Path]):
    """
    Save extracted SIFT features (keypoints and descriptors) to separate subdirectories.
    ...
    for image_name, keypoints, descriptors in features:
        ...
        with open(keypoints_dir / f"{base_name}_sift_keypoints.json", 'w') as f:
            json.dump(keypoints_data, f, indent=2)

        # Save descriptors
        np.save(descriptors_dir / f"{base_name}_sift_descriptors.npy", descriptors)
...
def main(source: Union[str, Path],
         ...
         output_dir: Union[str, Path] = 'sift_features_output'):
    """
    Main function to run the SIFT feature extraction pipeline.
    ...
    features = extract_sift_features(images, num_features)
    save_features(features, output_dir)
    print(f"Extracted and saved SIFT features for {len(features)} images.")
...
if __name__ == "__main__":
    ...
    main(source='/home/thales1/VSLAM/Vslam_algorithm/pythonProject1/slam01_2/data',
         ...
         resize_dim=(640, 480),
         num_features=1000,
         output_dir='sift_features_output')